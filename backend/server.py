...
# ---------- Admin bulk-upload donations via CSV ----------
...
async def donations_csv_template(_: dict = Depends(admin_tab_dep("causes"))):
    """Download a CSV template — header row + one realistic example row so the
    admin can copy/paste their own data into the same structure."""
    import csv as _csv
    import io as _io
    buf = _io.StringIO()
    w = _csv.writer(buf)
...
async def donations_csv_import(
    ...
    `cause` (optional title; matched case-insensitively against db.causes;
    blank ⇒ general "Unallocated"), `date` (optional, parsed via
    `_parse_csv_date_local`; defaults to today), `note`, `anonymous`
    (true/false), `method` (free-text, defaults to `manual_csv`).

    `dry_run=true` returns a row-by-row preview (READY/ERROR) without writing
    anything. `dry_run=false` writes `db.transactions` records (status=completed,
    type=donation) and refreshes `causes.raised_amount`/`donor_count`.
    Returns `{ok, created, failed, total, rows: [...preview...]}`.
    """
    import csv as _csv
    import io as _io
    from routes.hours import _parse_csv_date as _parse_csv_date_local  # reuse the hours module's flexible date parser
    ...
    for idx, raw_row in enumerate(reader, start=2):  # row 2 = first data row (header is row 1)
        ...
        if email:
            u = await db.users.find_one(
                {"email": {"$regex": f"^{_re_escape_local(email)}$", "$options": "i"}},
    ...
    if not dry_run and valid_writes:
        ...
        # Refresh raised_amount + donor_count for any cause that received money.
...
def _re_escape_local(s: str) -> str:
    """Tiny re.escape shim so we don't have to add `import re` at module top
    (it is already imported in this file but keeping a local helper avoids
    accidental shadowing during the future server.py extraction)."""
    import re as _re
    return _re.escape(s)
...
async def leaderboard_top_donors(period: str = "quarter", user: dict = Depends(get_current_user)):
    ...
    member_pipe = [
        {"$match": match},
        # Exclude anonymous from member leaderboard (privacy)
