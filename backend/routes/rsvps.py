    ...
    *,
    ...
    async def admin_rsvp_csv_template(event_id: str, admin: dict = Depends(get_current_user)):
        ...
        sample_with_guests = [
            ...
            "Alex Plus-one; Pat Friend",  # semicolon-separated names
            "general; vip",                # parallel list — same length as guests, optional
        ]
    ...
    async def admin_rsvp_csv(
        ...
        # Pre-resolve every email + every existing RSVP in one round-trip each.
        ...
        preview: list[dict] = []
        errors: list[dict] = []
        ready_docs: list[dict] = []  # {target_user, ticket_type, normalized_guests}
        ...
        for idx, row in enumerate(rows, start=2):
            ...
            if capacity > 0 and running_seats + seats_needed > capacity:
                ...
                continue
            # Dedupe: same member email appearing twice in the same file
